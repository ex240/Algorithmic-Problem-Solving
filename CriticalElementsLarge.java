...
public class CriticalElementsLarge {
	...
	public static void main(String[] args) throws IOException {
			...
			for (int j = i-1; j >= 0; j--) {
				if (A[j] < A[i]) {
					if (Lfrom[i] + 1 > Lfrom[j]) {
						...
						numLfrom[j] = (numLfrom[j] + numLfrom[i]) % M;
					}
				}
			}
		}
		...
		for (int i = 0; i < n; i++) {
			if (Lto[i] == maxLen) maxNum = (maxNum + numLto[i]) % M;
		}
		...
		String critical = "";
		for (int i = 0; i < n; i++) {
			if (Lto[i] + Lfrom[i] - 1 == maxLen && ((numLto[i] * numLfrom[i]) % M) == maxNum) {
				critical += A[i] + " ";
			}
		}
